...
async function loadStrokeData(character) {
  ...
  try {
    ...
    state.totalStrokeWeight = strokes.reduce((sum, stroke) => sum + stroke.weight, 0);
    state.isLoading = false;
    syncIntervalLimit();
    saveSettings();
    setDataStatus(`${strokes.length}画の正方向データを反転`, "ready");
    setCanvasMessage();
    updateUI();
    render();
  } catch (error) {
...
function applyCharacterTransform(ctx) {
  const scale = Math.min(800 / SOURCE_WIDTH, 800 / SOURCE_WIDTH);
  const x = OUTPUT_WIDTH / 2 - SOURCE_WIDTH * scale / 2;
  const y = OUTPUT_HEIGHT / 2 + SOURCE_BASELINE * scale / 2;
...
function drawPartialStroke(stroke, amount) {
  ...
  strokeContext.clearRect(0, 0, OUTPUT_WIDTH, OUTPUT_HEIGHT);
...
function updateUI() {
  ...
  if (state.isLoading) {
    elements.stageStatus.textContent = "正方向データを読み込み中";
  } else if (!hasData) {
    elements.stageStatus.textContent = "画データなし";
  } else {
    const active = activeStrokeNumber();
    elements.stageStatus.textContent = active
      ? `${state.strokes.length}画 · 第${active}画を終点から描画`
      : `${state.strokes.length}画 · 完成`;
  }
}
...
async function exportVideo() {
  ...
  anchor.href = url;
  anchor.download = `${safeFileCharacter()}-reverse-kanji.webm`;
  anchor.click();
...
function exportImage() {
  ...
  elements.canvas.toBlob((blob) => {
    ...
    const anchor = document.createElement("a");
    anchor.href = url;
    anchor.download = `${safeFileCharacter()}-reverse-kanji-${Number(elements.imageTime.value).toFixed(1)}s.png`;
    anchor.click();
    window.setTimeout(() => URL.revokeObjectURL(url), 1500);
